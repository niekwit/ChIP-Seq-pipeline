...
do
	...
	read2_val_2_fq_gz="${read1_fastq_gz%_2.fastq.gz}_2_val_2.fq.gz"
	bowtie2_output=${read1_fastq_gz%_1.fastq.gz}
	bowtie2_output=${bowtie2_output##*/}
	bowtie2 -p 40 -x $index_path -1 trim_galore/$read1_val_1_fq_gz -2 trim_galore/$read2_val_2_fq_gz 2>> chip-seq.log | samtools view -q 15 -bS - > "bam/${bowtie2_output}.bam" 2>> chip-seq.log
