...
index_path=$(cat "$SCRIPT_DIR/references.yaml" | shyaml get-value $align_setting.$genome)
blacklist_path=$(cat "$SCRIPT_DIR/references.yaml" | shyaml get-value blacklist.$genome)
...
elif [[ $align_prog == *"-pe"* ]]; then
	if [[ ! -d ./trim_galore ]]; then
		for file in raw-data/*_1.fastq.gz 
		do
			echo "Trimming fastq files"
...
if [[ $align_prog == *"hisat2-pe"* ]]; then
	...
	do
		...
		hisat2 -p "$threads" -x "$index_path" -1 $file -2 $file2 2>> align.log | samtools view -q 15 -F 260 -bS -@ "$threads" - | bedtools intersect -v -a "stdin" -b "$blacklist_path" -nonamecheck | samtools sort -@ "$threads" - > "$hisat2_output" #bam file will not contain unmapped and multimapped reads 
	done
...
then
	...
	do
		echo $file
		bwa_output="${file%1_val_1.fq.gz}-sort-bl.bam"
		bwa_output="bam/${hisat2_output##*/}"
		file2="${file%1_val_1.fq.gz}2_val_2.fq.gz"
		bwa mem "$index_path" -t "$threads" "$file" "$file2" | samtools view -q 15 -F 260 -bS -@ "$threads" - | bedtools intersect -v -a "stdin" -b "$blacklist_path" -nonamecheck | samtools sort -@ "$threads" - > "$bwa_output" #bam file will not contain unmapped and multimapped reads
	done
elif [[ $* == *"bwa-se"* ]]; 
then
	...
	do
		...
		bwa_input="trim_galore/${bwa_input##*/}"
		bwa_output="bam/${file%_1.fastq.gz}-sort-bl.bam"
		bwa mem $index_path -t $threads $bwa_input 2>> align.log | samtools view -q 15 -F 260 -bS -@ "$threads" - | bedtools intersect -v -a "stdin" -b "$blacklist_path" -nonamecheck | samtools sort -@ "$threads" - > "$bwa_output" #bam file will not contain unmapped and multimapped reads
	done
fi
...
do
	count=$(samtools view -@ $threads -c -F 260 $file) #bit 3 and 9 SAM FLAGs
