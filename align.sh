...
if [[ $* == *"hisat"* ]];
	...
	then
		...
		do
			...
			bwa mem $index_path -t $max_threads 2>> align.log | samtools view -q 15 -F 260 -bS -@ $max_threads - | bedtools intersect -v -a "stdin" -b $blacklist_path -nonamecheck | samtools sort -@ $max_threads - > $bwa_output #bam file will not contain unmapped and multimapped reads
		done
	fi
